...
@Component({
  selector: 'rmncha-group-bar-chart',
...
export class GroupBarChartComponent implements OnInit {
  ...
  createChart(data) {
    ...
    let color = ["#c26b61", "#F37361", "#F4A775", "#F4C667"];

    let hoverColor = ["#017A27", "#FF5900", "#b7191c","#F37361"];
    ...
    let xAxis = d3.axisBottom().scale(x).tickFormat(formatTick);
    ...
    let rect = layer.selectAll("rect").data(function (d) {
      return d;
    ...
    svg.append("g").attr("class", "x axis").attr("transform",
      "translate(0," + (height-margin.top) + ")").call(xAxis)
      ...
      .call(wrap, x.bandwidth(), width) .on("mouseover", function (d) {
        $(this).popover(
          {
            title: '',
            placement: 'top',
    ...
    svg.append("g").attr("class", "y axis").call(yAxis).append(
      "text").attr("transform", "rotate(-90)").attr("y",
        -28 - margin.left).attr("x", 0 - (height / 3.5)).attr(
      ...
      if (this.values) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          layer.selectAll("evmbartext" + i).data(data[i]).enter()
            ...
            .attr("y", function (d) {
              return y(d.value) - 3;
            }) .attr("font-family", "'Questrial', sans-serif").attr("text-anchor", "middle").attr("fill", "#000")
