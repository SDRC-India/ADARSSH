...
export class SpiderChartComponent implements OnInit {
  ...
  createChart(d) {
    ...
    let mouseOutcolor = ["#8FBBD9", "#FFBF87"];
    let hoverColor = ["#1F77B4", "#FF8C26"];

    let total = allAxis.length;
    let radius = cfg.factor
    ...
    for (let j = 0; j <= cfg.levels - 1; j++) {
      let levelFactor = cfg.factor * radius
        * ((j + 1) / cfg.levels);
      g.selectAll(".levels")
        .data(allAxis)
        .enter()
    ...
    let series = 0;
    let axis = g.selectAll(".axis").data(allAxis)
