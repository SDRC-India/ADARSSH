...
@Component({
  selector: 'rmncha-three-d-bar-chart',
...
export class ThreeDBarChartComponent implements OnInit {
  ...
  createChart(data) {
    ...
    let margin = {
      ...
      let depth = 100 - margin.front - margin.back;


      let xScale = d3.scaleBand().range([0, width]).padding(0.5),
      yScale = d3.scaleLinear().rangeRound([height-margin.top, 0]),
    ...
    let gridY = d3.scaleLinear().domain([0, max]).range(
      [height-margin.top, 0]);

      let chart = d3.select(el).append("svg").attr("id", "columnbarChart")
    .attr("width",
  ...
  .attr("transform", "translate(35," + (data[0][0].axis.length > 20 ? 12 : 15) + ")");
    ...
    let layer = chart.selectAll(".layer").data(layers).enter()
    ...
    let camera = [width / 2, height / 2, -200];
    let barGen = bar3d()
      .camera(camera)
      .x(x)
      .y(y)
