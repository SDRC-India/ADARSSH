...
import { ReviewPageModel } from '../interface/reviewpagemoel';

@Injectable({
  providedIn: 'root'
})
export class FormServicService {
  ...
  saveData(formId: any, dataModel: any) {
      ...
      return this.http.post(Constants.COLLECTION_SERVICE_URL+"saveData",dataModel,{responseType:'text'})
    
  }

  getDrafts(formId:any):Observable<ReviewPageModel> {
   
    return this.http.get<ReviewPageModel>(Constants.COLLECTION_SERVICE_URL+"getDataForReview?formId="+formId);
  ...
  getBlankQuestion():Observable<any>{
    return this.http.get<any>(Constants.COLLECTION_SERVICE_URL +"getQuestion");
  }
