...
export class TableComponent implements OnInit, OnChanges {
  ...
  @Output() onApproveSubmissionClicked: EventEmitter<any> = new EventEmitter<any>();
  ...
  ngOnInit() {
    ...
    this.lastURLSegment = pageName.substr(pageName.lastIndexOf('/') + 1);
  }
  ...
  ngOnChanges() { 
    ...
    if (localStorage.getItem(Constants.USER_DETAILS)) {
      this.userToken = JSON.parse(localStorage.getItem(Constants.USER_DETAILS));
    }
  ...
  public approveSubmissionByClicked(tableRow, i, pageNo) {
    this.selectedRowIndex = i;
    let emittedData: any = { pageno: pageNo, tableRow: tableRow};
    this.onApproveSubmissionClicked.emit(emittedData);
  }
