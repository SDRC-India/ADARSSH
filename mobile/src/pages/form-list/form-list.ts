  ...
  async findAllFroms() {
    ...
    await  this.formProvider.findAllFroms(this.formId).then(data => {
          if (data) {
            ...
            this.dateLineDate = this.deadlineDateModel.deadlineDays+"-"+b
            if(Number(a.split("-")[0]) > Number(this.deadlineDateModel.deadlineDays)){
            ...
            let revisedDate: string = this.deductDate(new Date(Number(this.dateLineDate.split("-")[2]), Number(this.dateLineDate.split("-")[1])-1, Number(this.dateLineDate.split("-")[0])+1), Number(this.deadlineDateModel.notifyByDays))
            ...
            if(currentDate >= startDate && currentDate <= endingDate){
                ...
                this.pendingForSyncCount = ifroms.filter(d => d.formStatus == "finalized" && (Number(d.visitedDate.split("-")[1]) == 12) && (Number(d.visitedDate.split("-")[2]) == Number(this.dateLineDate.split("-")[2])-1)).length
                for (let index = 0; index < ifroms.length; index++) {
                  if(ifroms[index].formStatus == "finalized" && (Number(ifroms[index].visitedDate.split("-")[1]) == 12) && (Number(ifroms[index].visitedDate.split("-")[2]) == Number(this.dateLineDate.split("-")[2])-1)){
                    ifroms[index].pendingForSyncRecord = true;
                  }
                }
              }else{
                this.pendingForSyncCount = ifroms.filter(d => d.formStatus == "finalized" && (Number(d.visitedDate.split("-")[1]) == Number(this.dateLineDate.split("-")[1])-1) && (Number(d.visitedDate.split("-")[2]) == Number(this.dateLineDate.split("-")[2]))).length
                for (let index = 0; index < ifroms.length; index++) {
                  if(ifroms[index].formStatus == "finalized" && (Number(ifroms[index].visitedDate.split("-")[1]) == Number(this.dateLineDate.split("-")[1])-1) && (Number(ifroms[index].visitedDate.split("-")[2]) == Number(this.dateLineDate.split("-")[2]))){
                    ifroms[index].pendingForSyncRecord = true;
                  }
                }
              }
            }else{
              this.pendingForSyncCount = 0
            }
            // })
