...
export class WebFormComponent {
  ...
  indexMap: {} = {};
  constructor(private constraintTokenizer: ConstraintTokenizer, 
    private commonsEngineProvider: CommonsEngineProvider, private applicationDetailsProvider: ApplicationDetailsProvider,
     public questionService: QuestionServiceProvider, public messageService: MessageServiceProvider,
      private navCtrl: NavController, public viewCtrl: ViewController, public datepipe: DatePipe,
       public formService: FormServiceProvider, public navParams: NavParams, 
       private dataSharingProvider: DataSharingServiceProvider, public modalCtrl: ModalController, 
       private datePickerProvider: DatePickerProvider, private engineUtilsProvider: EngineUtilsProvider,
        private atp: AmazingTimePickerService, private geolocation: Geolocation, private storage: Storage,
         private userService: UserServiceProvider, private alertCtrl: AlertController, private imagePicker: ImagePicker,
  ...
  chosenYearHandler(normalizedYear: Moment) {
    ...
    // this.date = ctrlValue
    // console.log("normalizedYear " + normalizedYear.year())
  }
  ...
  chosenMonthHandler(normlizedMonth: Moment, datepicker: MatDatepicker < Moment > ) {
    ...
    // console.log("normlizedMonth " + normlizedMonth.month())
    // datepicker.close();
  }
  ...
  onCameraFileChange($event, question) {
    ...
    let file = files[0];
    if (file && (file.name.split(".")[(file.name.split(".") as string[]).length - 1] as String).toLocaleLowerCase() === "png" || (file.name.split(".")[(file.name.split(".") as string[]).length - 1] as String).toLocaleLowerCase() === "jpg" || (file.name.split(".")[(file.name.split(".") as string[]).length - 1] as String).toLocaleLowerCase() === "jpeg") {
            ...
            switch (feature.split(":")[0]) {
              case "exp":
                for (let cols of feature.split(":")[1].split("&")) {
                  ...
                  if (result != null && result != "NaN" && result != NaN && result != "null" && cell.type == "tel") {
        ...
        case "filterByExp":
          {
            if (question.features != null && feature.includes("filterByExp") && !feature.includes("filterByExp:(")) {
              ...
              for (let option of this.questionMap[colName].options) {
                ...
                //  let facilityKey = this.questionMap['f1FacilityType'].value
                //  let districtKey = this.questionMap['f1q_district'].value
                //  let blockKey = this.questionMap['f1q_block'].value
                // console.log(modExp, '\n',
                //  this.questionMap['f1FacilityType'].value, '\n',
                //  JSON.stringify(option), '\n',
                //  JSON.stringify(this.questionMap['f1FacilityType'].options.find(element=>  element.key == facilityKey)) ,'\n',
                //  this.questionMap['f1q_district'].value,  '\n',
                //  JSON.stringify(this.questionMap['f1q_district'].options.find(element=>  element.key == districtKey)) ,'\n',
                //  this.questionMap['f1q_block'].value,  '\n',
                //  JSON.stringify(this.questionMap['f1q_block'].options.find(element=>  element.key == blockKey)) ,'\n',
                //  this.questionMap['f1q_block'].value,option['extraKeyMap'],  '\n',
                //  arithmeticExpression,isRelevantToDisplay)
