...
export class FormProvider {
  ...
  deleteSingleRecord(record: IDbFormModel, formId: any){
  ...
  */
  ...
  async getDeadlineDateInfo(formId): Promise<any>{
    let promise: Promise<any> = new Promise(async (resolve, reject)=>{
      await this.storage.get(ConstantProvider.dbKeyNames.deadLineDate).then(deadlineData=>{
        deadlineData = deadlineData[formId.split("_")[0]]
        resolve(deadlineData)
      })
      .catch(err=>{
        // console.log(err)
        reject(err.message)
  ...
  async getDeadlineDateInfoAll(): Promise<any>{
    let promise: Promise<any> = new Promise((resolve, reject)=>{
      this.storage.get(ConstantProvider.dbKeyNames.deadLineDate).then(deadlineData=>{
        resolve(deadlineData)
      })
      .catch(err=>{
        // console.log(err)
