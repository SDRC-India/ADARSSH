    ...
    for (let j = 0; j < data[Object.keys(data)[index]].length; j++) {
      ...
      for (let qs = 0; qs < Object.keys(subSections).length; qs++) {
        for (let q = 0; q < subSections[Object.keys(subSections)[qs]].length; q++) {
          let question: IQuestionModel = subSections[Object.keys(subSections)[qs]][q]
          let mandatory = false
          if ((type == "save" && question.saveMandatory)) {
            mandatory = true
          }
          else if ((type == "finalized" && question.finalizeMandatory)) {
            mandatory = true
          }
          if (mandatory) {
            switch (question.controlType) {
              case "geolocation":
                if (mandatory == true && question.displayComponent == true && (question.value == null || question.value == "")) {
                  service.displayError(question, index, "Please enter " + question.label)
                  break loop1;
                }
                break;
              case "camera":
              case "file":
                if (this.applicationDetailsProvider.getPlatform().isAndroid) {
                  if (mandatory == true && question.displayComponent == true && !(question.value)) {
                    service.displayError(question, index, "Please choose " + question.label)
                    break loop1;
                  }
                } else {
                  if (mandatory == true && question.displayComponent == true && question.attachmentsInBase64.lenth == 0) {
                    service.displayError(question, index, "Please choose " + question.label)
                    break loop1;
                  }
                }
              ...
              case 'dropdown':
                ...
                if (mandatory == true && question.displayComponent == true && (question.value == null || question.value.length == 0)) {
                  service.displayError(question, index, "Please select " + question.label)
                  break loop1
              ...
              case 'textbox':
                if (mandatory == true && question.displayComponent == true && (question.value == null || question.value == '')) {
                  service.displayError(question, index, "Please fill " + question.label)
                  break loop1
                }
                break;
              case "Time Widget":
                  ...
                  for (let j = 0; j < Object.keys(tableData[i]).length; j++) {
                    ...
                    if (typeof cell == 'object') {
                      if (mandatory == true && cell.displayComponent == true && (cell.value == null || cell.value == '')) {
                        service.displayError(cell, index, "Please enter " + question.label + " " + (cell.label).replace('@@split@@', ''), "table")
                        break loop1;
                      }
                    }
                  }
                    ...
                    switch (beginRepeatQuestion.controlType) {
                      case "geolocation":
                        if (mandatory == true && beginRepeatQuestion.displayComponent == true && (beginRepeatQuestion.value == null || beginRepeatQuestion.value == "")) {
                          service.displayError(beginRepeatQuestion, index, "Please select " + beginRepeatQuestion.label)
                          break loop1;
                        }
                        break;
                      case "camera":
                        if (mandatory == true && beginRepeatQuestion.displayComponent == true && beginRepeatQuestion.value) {
                          service.displayError(beginRepeatQuestion, index, "Please enter " + beginRepeatQuestion.label)
                          break loop1;
                        }
                        break;
                      case "file":
                        if (mandatory == true && beginRepeatQuestion.displayComponent == true && beginRepeatQuestion.value) {
                          service.displayError(beginRepeatQuestion, index, "Please choose " + beginRepeatQuestion.label)
                          break loop1;
                        }
                        break;
                      case 'dropdown':
                      ...
                      case 'textbox':
                        if (mandatory == true && beginRepeatQuestion.displayComponent == true && (beginRepeatQuestion.value == null || beginRepeatQuestion.value.trim() == '')) {
                          service.displayError(beginRepeatQuestion, index, "Please enter a valid " + beginRepeatQuestion.label)
                          break loop1
                        }
                        break;
                      case 'Date Widget':
